...
class TestSettingsFiles:
    ...
    def test_should_be_possible_to_set_value_which_is_absent_in_json_file(
        ...
        key = "absentvalue"
        assert_that(
            get_profile.is_value_present(key),
            equal_to(False),
    ...
    def test_should_be_possible_to_get_value(self, get_profile):
        ...
        assert_that(
            language,
            equal_to(TestValues.EXPECTED_LANGUAGES[language_key]),
            f"Logger language in settings file {TestValues.FILE_NAME} should be read correctly",
        )
        ...
        assert_that(
            language,
            equal_to(new_lang),
            f"Logger language in settings file {TestValues.FILE_NAME} "
    ...
    def test_should_be_possible_to_get_list_of_values(self, get_profile):
        ...
        assert_that(
            ...
            f"List of values in settings file {TestValues.FILE_NAME} should be read correctly",
        )

        expected_arguments = ["firstNew", "secondNew"]
        new_args = "firstNew,secondNew"
        os.environ[TestValues.ARGUMENTS_ENV_KEY] = new_args
        arguments = get_profile.get_list(arguments_path)
        assert_that(arguments, not_none(), "Arguments list is none")
        assert_that(
            arguments,
            equal_to(expected_arguments),
            f"Value in list in settings file {TestValues.FILE_NAME} be overridden with environment variable",
        )
    ...
    def test_should_be_possible_to_get_map(self, get_profile):
        ...
        assert_that(
            languages,
            equal_to(TestValues.EXPECTED_LANGUAGES),
            f"Dictionary of values in settings file {TestValues.FILE_NAME} should be read correctly",
        )

        new_language_value = "newLangMap"
        expected_languages = {"language": new_language_value}
        os.environ[TestValues.LANGUAGE_ENV_KEY] = new_language_value
        languages = get_profile.get_dictionary(logger_path)
        ...
        assert_that(
            ...
            f"Dictionary of values in settings file {TestValues.FILE_NAME} "
            f"should be overridden with environment variable",
        )

    def test_should_be_possible_to_check_is_value_present(self, get_profile):
        key = TestValues.TIMEOUT_POLLING_INTERVAL_KEY
        is_time_out_present = get_profile.is_value_present(key)

        assert_that(
            is_time_out_present,
            f"{key} value should be present in settings file {TestValues.FILE_NAME}",
        )
        ...
        assert_that(
            ...
            equal_to(False),
            f"{wrong_path} value should not be present in settings file {TestValues.FILE_NAME}",
        )
