...
class TestsSettingsFiles:
    ...
    def test_should_be_possible_to_override_boolean_value_via_environment_variable(self, get_test_profile):
        ...
        assert_that(bool(strtobool(get_test_profile.get_value(TestKeys.BOOLEANVALUE_ENV_KEY))),
                    equal_to(target_value),
                    'value passed via env var is not used by SettingsFile')

    def test_should_be_possible_to_set_value_which_is_absent_in_json_file(self, get_test_profile):
        assert_that(not get_test_profile.is_value_present(TestKeys.ABSENTVALUE_PATH),
                    'value should be absent by default')
        target_value: str = str(True)
        os.environ[TestKeys.ABSENTVALUE_PATH] = target_value
        assert_that(get_test_profile.is_value_present(TestKeys.ABSENTVALUE_PATH), 'value should be present after set')
        assert_that(get_test_profile.get_value(TestKeys.ABSENTVALUE_PATH), equal_to(target_value),
                    'value passed via env var is not used by SettingsFile')
    ...
    def test_should_be_possible_to_get_default_content(self, get_default_profile):
        ...
        assert_that(language, equal_to('en'), 'Logger language in default settings file should be read correctly')
    ...
    def test_should_be_possible_to_get_value(self, get_test_profile):
        ...
        assert_that(language, equal_to(TestKeys.EXPECTED_LANGUAGES[language_key]),
    ...
    def test_should_be_possible_to_get_list_of_values(self, get_test_profile):
        ...
        assert_that(arguments, equal_to(expected_arguments),
    ...
    def test_should_be_possible_to_get_map(self, get_test_profile):
        ...
        assert_that(languages, equal_to(TestKeys.EXPECTED_LANGUAGES),
        ...
        assert_that(languages is not None)
        assert_that(languages, equal_to(expected_languages),
                    f'Dictionary of values in settings file {TestKeys.FILE_NAME} should be overridden with environment variable')

    def test_should_be_possible_to_check_is_value_present(self, get_test_profile):
        is_time_out_present: bool = get_test_profile.is_value_present(TestKeys.TIMEOUT_POLLING_INTERVAL_PATH)

        assert_that(is_time_out_present,
        ...
        assert_that(not is_wrong_path_present,
                    f'{wrong_path} value should not be present in settings file {TestKeys.FILE_NAME}')

    def test_should_be_possible_to_check_that_null_value_is_present(self, get_test_profile):
        is_null_value_present = get_test_profile.is_value_present(TestKeys.NULLVALUE_PATH)
    ...
    def test_should_throw_exception_when_value_not_found(self, func, get_test_profile):
        wrong_path: str = "blabla"
        assert_that(calling(getattr(get_test_profile, func.__name__)).with_args(wrong_path),
